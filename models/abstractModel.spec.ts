...
describe('Abstract Base Model', () => {
    ...
    it('should be able to retrieve a model\'s attributes excluding some properties', () => {

        let model = new TestModel({
            foo: 'bar',
        }, false);

        expect(model.getAttributes(false, ['bar'])).to.deep.equal({
            foo: 'bar',
        });
    ...
    it('should be able to convert a string to a slug', () => {

        let model = new TestModel({});
        let slugA = model.slugify('This is a Test');
        let slugB = model.slugify('This is another  test!');
        let slugC = model.slugify('  :<>"{thIS\'   iS A *(test 1+=  ');

        expect(slugA).to.equal('this-is-a-test');
        expect(slugB).to.equal('this-is-another-test');
        expect(slugC).to.equal('this-is-a-test-1');
    });
